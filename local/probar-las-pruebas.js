...
const MUTACIONES = [
  ...
  ['da por pagada una seña del curso', CODIGO,
   'if (cobrado >= PRECIOS.cursoTotal) {', 'if (cobrado > 0) {'],
  ...
  ['la nota del curso confunde cuota con seña', CODIGO,
   '} else if (cobrado % PRECIOS.cursoCuota === 0) {', '} else if (cobrado % PRECIOS.cursoCuota !== 0) {'],
  ...
  ['"Falta cobrar" suma lo cobrado en vez del saldo', CODIGO,
   "saldo: sumar(cobrando, 'saldo'),", "saldo: sumar(cobrando, 'recaudado'),"],
  ...
  ['los pendientes cuentan ediciones que ya pasaron', CODIGO,
   'pendientes: cobrando.reduce(', 'pendientes: ediciones.reduce('],
